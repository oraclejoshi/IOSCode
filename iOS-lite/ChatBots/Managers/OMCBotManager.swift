...
let kUsername = "botUsername"

class OMCBotManager: NSObject, WebSocketDelegate {
    
    let notifRequestIdentifier = "WSMessegeNotif" // identifier is to cancel the notification request

    static let sharedInstance = OMCBotManager()
    ...
    func establishConnection( host:String, userName:String ) -> Void {
        ...
        if( host.range(of: "://")?.isEmpty == false ){
            ...
            socket = WebSocket(url: URL(string: "ws://\(_host)/ext/apps/chat/ws?user=\(userName)")!, protocols: [])
        }
        else {
            socket = WebSocket(url: URL(string: "ws://\(host)/ext/apps/chat/ws?user=\(userName)")!, protocols: [])
        }
    ...
    func playBotsReply (chatMessege: NSDictionary? ) -> Void {
        ...
        //NOTE: Will come back later for fix on this. Speech to text is unchanged and works appropriatly.
        return;
    ...
    func messegeRecieved ( chatMessege:NSDictionary? ) -> Void {
        ...
        if chatMessege == nil {
            ...
            /*
            // Disabled for now as needs update for webhook 1.1 different formats support.
...
extension OMCBotManager:UNUserNotificationCenterDelegate {
